...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.filter(item => item.id === programmerId)[0].name
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (programmerId > 0) ? 'gold' : 'royalblue' // 🤔
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(item =>
            <div key={item.id}>
              {item.name} <button onClick={() => { setProgrammerId(item.id) }}>Feature</button>
            </div>
          )
        }
      </div>
